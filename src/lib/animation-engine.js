...
export function createAnimationEngine(root, onResize = () => {}) {
  ...
  const MobileMenu = {
    ...
    open() {
      ...
      this.menuIcons.forEach((icon, index) => {
        const direction =
          index === 0 ? 50 + this.gapOffset : -50 - this.gapOffset;
        gsap.to(icon, {
          yPercent: direction,
          duration: 0.4,
          ease: "power2.out",
  ...
  const GhostEngine = {
    ...
    destroy() {
      ScrollTrigger.getAll().forEach((st) => {
        ...
        if (
          trigger === ".hero" ||
          (typeof trigger === "string" && trigger.includes("hero"))
        ) {
          st.kill();
        }
      });
    ...
    getSettings(el) {
      ...
      if (configEl) {
        if (configEl.getAttribute("data-flip-trigger"))
          trigger = configEl.getAttribute("data-flip-trigger");
        if (configEl.getAttribute("data-flip-start"))
          start = configEl.getAttribute("data-flip-start");
        if (configEl.getAttribute("data-flip-end"))
          end = configEl.getAttribute("data-flip-end");
      }
    ...
    measurePair(real, ghost, type) {
      ...
      if (type === "text_font") {
        measurement.ghostFontSize = parseFloat(
      ...
      if (type === "background") {
        ...
        measurement.ghostBorderRadius =
          parseFloat(ghostStyle.borderRadius) || 0;
      }
    ...
    applyAnimation(m) {
      ...
      if (type !== "logo") {
        delete fromVars.width;
        delete toVars.width;
        delete fromVars.height;
        delete toVars.height;
      }
      // Only background type uses visual properties
      if (type !== "background") {
        delete fromVars.backgroundColor;
        delete toVars.backgroundColor;
        delete fromVars.borderRadius;
        delete toVars.borderRadius;
        delete fromVars.borderTopLeftRadius;
        delete toVars.borderTopLeftRadius;
        delete fromVars.borderTopRightRadius;
        delete toVars.borderTopRightRadius;
        delete fromVars.borderBottomLeftRadius;
        delete toVars.borderBottomLeftRadius;
        delete fromVars.borderBottomRightRadius;
        delete toVars.borderBottomRightRadius;
      ...
      delete fromVars.fontSize;
      delete toVars.fontSize;
      delete fromVars.color;
      delete toVars.color;
      // Only logo type uses absolute positioning
      if (type !== "logo") {
        delete fromVars.position;
        delete toVars.position;
        delete fromVars.top;
        delete toVars.top;
        delete fromVars.left;
        delete toVars.left;
      }

      delete fromVars.backdropFilter;
      delete toVars.backdropFilter;
      delete fromVars.webkitBackdropFilter;
      delete toVars.webkitBackdropFilter;
    ...
    createAll() {
      ...
      Utils.$$(".hero-navigation-link").forEach((link) => {
        ...
        measurements.push(
          this.measurePair(
            link,
            Utils.$(`.hero-link-ghost[data-link-id="${id}"]`),
            "link",
          ),
        );
      });
  ...
  const StyleEngine = {
    ...
    setupMaskedNumber(el) {
      ...
      const value =
        el.getAttribute("data-number-count") || el.textContent.trim();
      el.textContent = "";
        ...
        if (Utils.isSplit(el)) {
          targets =
            splitType === "lines"
              ...
              : el.querySelectorAll(`.${splitType.slice(0, -1)}`);
        } else {
  ...
  const ThemeSwitcher = {
    elements: [
      ...
      },
      {
        ...
        onDark: () => {
          Utils.$$(".nav-item-bg").forEach((el) =>
            el.classList.add("nav-item-dark"),
          );
        },
        onLight: () => {
          Utils.$$(".nav-item-bg").forEach((el) =>
            el.classList.remove("nav-item-dark"),
          );
        },
      },
      {
      ...
      },
      {
        ...
        onDark: () => {
          Utils.$$(".social-link").forEach((el) =>
            el.classList.add("social-dark"),
          );
        },
        onLight: () => {
          Utils.$$(".social-link").forEach((el) =>
            el.classList.remove("social-dark"),
          );
        },
      },
    ],
  ...
  const Preloader = {
    ...
    init() {
      ...
      const splitLines = (el, useMask = true) => {
        if (!el || typeof SplitText === "undefined" || Utils.isSplit(el))
          return [];
      ...
      if (navLinkInners.length) {
        this.timeline.to(
          navLinkInners,
          { yPercent: 0, duration: linkDur, ease: "power2.out" },
          navStart,
        );
        this.timeline.set(
          navLinks,
      ...
      if (navSeps.length) {
        this.timeline.to(
          navSeps,
          { height: "0.8vw", duration: sepDur, ease: "power2.out" },
          navStart,
        );
      }
  ...
  const MagneticPositions = {
    ...
    init() {
      ...
      Utils.addEvent(window, "resize", this._resizeHandler, { passive: true });
    ...
    collectPairs() {
      document.querySelectorAll("[data-origin]").forEach((target) => {
        if (target.hasAttribute("data-desktop") && window.innerWidth < 768)
          return;
    ...
    positionAllOnce() {
      this.pairs.forEach((pair) => {
        const { target, anchor, originConfig, anchorConfig, offsetX, offsetY } =
          pair;
        ...
        const finalX =
          Math.round((anchorPoint.x - originPoint.x + offX) * 100) / 100;
        const finalY =
          Math.round((anchorPoint.y - originPoint.y + offY) * 100) / 100;
      ...
      cards.forEach((card, index) => {
        ...
        Utils.addEvent(card, "click", function (e) {
          ...
          if (cloneArrowWrap) {
            ...
            Utils.addEvent(cloneArrowWrap, "click", function (e) {
              e.stopPropagation();
              self.closeActiveCard();
            });
          }
  ...
  const CTAAnimation = {
    init() {
      ...
      const typingBubble =
        typingGrid?.querySelector(".cta-bubble") ||
        typingGrid?.firstElementChild;
      const chatGrid = wrap.querySelector(".cta-chat-grid");
      const chatBubble =
        chatGrid?.querySelector(".cta-bubble") || chatGrid?.firstElementChild;
      const chatText = chatGrid?.querySelector(".cta-text");
      const buttonGrid = wrap.querySelector(".cta-button-grid");
      const buttonBubble =
        buttonGrid?.querySelector(".cta-bubble") ||
        buttonGrid?.firstElementChild;
  ...
  const SwiperInit = {
    ...
    init() {
          ...
          touchMove: (swiper, event) => {
            ...
            if (Math.abs(deltaX) > 5 && Math.abs(deltaX) > deltaY * 0.5) {
              ...
              if (newDirection !== this.lastDragDirection) {
                ...
                const leftIcon =
                  this.dragWrap?.querySelector(".drag-left-icon");
                const rightIcon =
                  this.dragWrap?.querySelector(".drag-right-icon");
      ...
      if (swiperEl) {
        ...
        const dragParagraphs = this.dragWrap
          ? this.dragWrap.querySelectorAll("p")
          : [];

        if (clientLinks.length && dragParagraphs.length >= 2) {
          clientLinks.forEach((link) => {
            Utils.addEvent(link, "mouseenter", () => {
              gsap.to(dragParagraphs[0], {
                yPercent: -100,
                duration: 0.4,
                ease: "power2.out",
              });
              gsap.to(dragParagraphs[1], {
                yPercent: -100,
                duration: 0.4,
                ease: "power2.out",
              });
  ...
  const LenisInit = {
    init() {
      ...
      this.ticker = (time) => STATE.lenis?.raf(time * 1000);
  ...
  const ImageTrail = {
    images: [
      "/assets/41ca054d-frame-116046298.avif",
      "/assets/dc998a4c-frame-116046247.avif",
      "/assets/0e9270ab-frame-116046251.avif",
      "/assets/17c545ef-frame-116046250.avif",
      "/assets/29e667fc-frame-116046242.avif",
      "/assets/816743aa-frame-116046243.avif",
      "/assets/a54d802c-frame-116046248.avif",
      "/assets/8873cbb1-frame-116046241.avif",
    ],
    config: {
    ...
    addImage(x, y) {
      ...
      img.setAttribute(
        "href",
        this.images[this.imageIndex++ % this.images.length],
      );
  ...
  return {
    ...
    finishIntro() {
      if (Preloader.timeline && Preloader.timeline.progress() < 1) {
        Preloader.timeline.progress(1);
        ScrollTrigger.refresh();
      }
    },
    closeCapability() {
      CardInteractions.closeActiveCard();
    },
    get lenis() {
      return STATE.lenis;
    },
    destroy() {
      ...
      context?.revert();
      root
        .querySelectorAll(".clone-text,.clone-p")
